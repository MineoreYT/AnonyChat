...
export default function App() {
  ...
  const selectChat = user => {
    ...
    if (user !== "LIVE") setUnreadCounts(prev => ({ ...prev, [user]: 0 }));
    scrollToBottom();
  };
  ...
  const LiveBubble = ({ m }) => (
    <div style={{ marginBottom:12 }}>
      <strong style={{ color: avatarColor(m.userID) }}>{m.userID}</strong>
      <span style={{ opacity:0.5 }}> [{m.time}]</span>
      <div style={{ wordBreak:"break-word", overflowWrap:"anywhere", maxWidth:"100%" }}>{m.text}</div>
    </div>
  );
        ...
        <div style={{ marginTop:8, overflowY:"auto", flex:1 }}>
          {onlineUsers.map(u=>(
            <div key={u} onClick={()=>selectChat(u)} style={{ padding:12, borderRadius:12, background:selectedUser===u?"#4a90e2":"#242424", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
              ...
              {unreadCounts[u]>0 && <span style={{ background:"red", padding:"4px 8px", borderRadius:20, fontSize:12 }}>{unreadCounts[u]}</span>}
            </div>
          ))}
        </div>
      </div>
